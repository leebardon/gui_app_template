...
class Tasks:
    ...
    def update(self, message):
        self._tasknum += 1
        self._message = message
        print(self._tasknum, self._message)
...
def launch_pb2(tasks):
    ...
    sg.theme("DarkTeal2")
    progressbar = [
        [sg.ProgressBar(tasks._maxtasks, orientation="h", size=(51, 10), key="progress_1")]
    ]
    ...
    main_window['progress_1'].update(tasks._tasknum)

    threading.Thread(target=update_progress,
                    args=(main_window, tasks := tasks),
                    daemon=True).start()
